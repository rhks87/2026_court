    ...
    for(let c=0;c<7;c++){
      ...
      if(isBlank){
        ...
        if(wx){
          ...
          html+=`<div class="day-wx" title="${wxTipTxt}" onclick="showWxTip(event,'${wxTipTxt.replace(/'/g,"\\'")}' )">${icon}<span class="dwx-txt"> ${wx.tmax}°<span class="dwx-pop"> 💧${wx.pop}%</span></span></div>`;
        }
